...
describe('useFirestore Business Logic Tests', () => {
  ...
  describe('Data Validation', () => {
    ...
    it('should validate mesure data structure with BMI calculation', () => {
      ...
      const imc = Math.round((validMesureData.poids / Math.pow(validMesureData.taille / 100, 2)) * 100) / 100
  ...
  describe('Data Processing Functions', () => {
    it('should process repas macros correctly', () => {
      const aliments = [
        { nom: 'Pain', quantite: 100, unite: 'g', macros: { kcal: 250, proteines: 8, glucides: 45, lipides: 2 } },
        { nom: 'Beurre', quantite: 10, unite: 'g', macros: { kcal: 75, proteines: 0, glucides: 0, lipides: 8 } }
      ]

      // Calculer les macros totales comme le ferait le hook
      const totalMacros = aliments.reduce((total, aliment) => ({
    ...
    it('should calculate training duration correctly', () => {
      ...
      const totalDuration = trainings.reduce((total, training) => total + training.duree, 0)
  ...
  describe('Error Handling', () => {
    it('should handle invalid repas data gracefully', () => {
      ...
      const isValidNom = !!(invalidData.nom && invalidData.nom.trim().length > 0)
      const isValidDate = invalidData.date && !isNaN(Date.parse(invalidData.date))
      const isValidAliments = Array.isArray(invalidData.aliments)
      const isValidMacros = !!(invalidData.macros && typeof invalidData.macros === 'object')
    ...
    it('should validate required fields', () => {
      ...
      const repasValid = testData.repas.nom && testData.repas.date && testData.repas.aliments !== null
      const entrainementValid = testData.entrainement.type && testData.entrainement.duree > 0
      const mesureValid = testData.mesure.poids > 0 && testData.mesure.taille > 0
  ...
  describe('Integration Scenarios', () => {
    it('should handle complete user workflow', () => {
      ...
      const processedWorkflow = {
        ...
        entrainement: { ...userWorkflow.entrainement, user_id: userWorkflow.user.uid },
