...
export function useAriaAnnouncer() {
  ...
  const announceValidationError = useCallback((fieldName: string, error: string) => {
    announce(`Erreur dans le champ ${fieldName}: ${error}`, { priority: 'assertive' })
  }, [announce])
  ...
  const announceModalState = useCallback((isOpen: boolean, modalTitle: string) => {
    const state = isOpen ? 'ouverte' : 'fermée'
    announce(`Modal ${modalTitle} ${state}`, { priority: 'assertive' })
  }, [announce])

  // Announce data updates
  const announceDataUpdate = useCallback((dataType: string, action: string) => {
