...
import {
  ...
  getSportCorrectionFactor,
} from '@/lib/userCalculations';
...
export function useEnergyBalance({
  ...
  return useMemo(() => {
    ...
    const rawSportCalories = entrainements.reduce(
    ...
    const correctionFactor = userProfile?.niveau_activite
      ? getSportCorrectionFactor(userProfile.niveau_activite)
      : 0.7;

    // TDEE ajusté (avec pondération)
    const adjustedTDEE = userProfile
      ? calculateAdjustedTDEE(userProfile, avgDailySportCalories) || baseTDEE
      : baseTDEE;

    // Calories sport pondérées
    const adjustedSportCalories = Math.round(
      rawSportCalories * correctionFactor,
    );
