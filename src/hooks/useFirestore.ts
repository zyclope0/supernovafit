...
export function useRepas() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      q,
      (snapshot) => {
        const repasData = snapshot.docs.map(
  ...
  const updateRepas = async (
    id: string,
    repasData: Partial<Omit<Repas, 'id' | 'created_at'>>,
  ) => {
    if (!user) return { success: false, error: 'Non connecté' };
...
export function useEntrainements() {
  ...
  const addEntrainement = async (
    entrainementData: Omit<Entrainement, 'id'>,
  ) => {
    if (!user) {
  ...
  const updateEntrainement = async (
    id: string,
    entrainementData: Partial<Omit<Entrainement, 'id' | 'user_id'>>,
  ) => {
    if (!user) return { success: false, error: 'Non connecté' };
  ...
  const addToFavoris = async (aliment: Omit<Aliment, 'id'>) => {
    ...
    const exists = favoris.some(
      (f) =>
        ...
        (aliment.openfoodfacts_id &&
          f.openfoodfacts_id === aliment.openfoodfacts_id),
    );
...
export function useMesures() {
  ...
  const updateMesure = async (id: string, mesureData: Partial<Mesure>) => {
    ...
    try {
      ...
      await updateDoc(
        doc(db, 'mesures', id),
        dataToUpdate as Partial<DocumentData>,
      );
      return { success: true };
    } catch (error: unknown) {
      return {
  ...
  const getStats = (mesure: Mesure): MesureStats | null => {
    ...
    if (derniereMesure?.poids) {
      evolution_poids =
        ((mesure.poids - derniereMesure.poids) / derniereMesure.poids) * 100;
    }
    if (derniereMesure?.masse_grasse && mesure.masse_grasse) {
      evolution_masse_grasse =
        mesure.masse_grasse - derniereMesure.masse_grasse;
    }

    // Tendances (simplifié pour l'instant)
    const tendance_7j: 'hausse' | 'baisse' | 'stable' =
      Math.abs(evolution_poids) < 1
        ? 'stable'
        : evolution_poids > 0
          ? 'hausse'
          : 'baisse';
...
export function useJournal() {
  ...
  const updateEntry = async (
    id: string,
    entryData: Partial<Omit<JournalEntry, 'id' | 'user_id' | 'created_at'>>,
  ) => {
    if (!user) return { success: false, error: 'Non connecté' };
...
export function useAllAthletes() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        console.error(
          '❌ FIRESTORE - Erreur récupération tous les athlètes:',
          error,
        );
        setAthletes([]);
        setLoading(false);
...
export function useCoachAthletes() {
  ...
  type AthleteLite = Pick<
    UserProfile,
    'id' | 'nom' | 'email' | 'objectif' | 'dernier_acces'
  > & { coach_id?: string };
  const [athletes, setAthletes] = useState<AthleteLite[]>([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        console.error(
          '❌ FIRESTORE - Erreur récupération athlètes coach:',
          error,
        );
        setAthletes([]);
        setLoading(false);
  ...
  const getAthleteData = async (athleteId: string) => {
    ...
    try {
      ...
      if (athleteDoc.exists() && athleteDoc.data().ownerCoachId === user.uid) {
        ...
        const repas = repasSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
        ...
        const entrainements = entrainementsSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
        ...
        const mesures = mesuresSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
...
type EntrainementAvecCalories = FirestoreEntrainement & {
    ...
    const fetchAthleteRealData = async () => {
      try {
        ...
        const proteines_jour = todayMeals.reduce(
          (sum: number, meal: FirestoreRepas) => sum + (meal.macros?.prot || 0),
          0,
        );
        ...
        const variation_perf =
          entrainementsMois.length > 0
            ? Math.min(entrainementsMois.length * 5, 50)
            : 0;
        ...
        for (let i = 6; i >= 0; i--) {
          ...
          const calories = repasDuJour.reduce(
            (sum: number, meal: FirestoreRepas) =>
              sum + (meal.macros?.kcal || 0),
            0,
          );
          const proteines = repasDuJour.reduce(
            (sum: number, meal: FirestoreRepas) =>
              sum + (meal.macros?.prot || 0),
            0,
          );
...
export function useCoachCommentsByModule(
  module: string,
  date?: string,
  itemId?: string,
) {
    ...
    (async () => {
      try {
        ...
        if ((module === 'entrainements' || module === 'journal') && itemId) {
          const idField =
            module === 'entrainements' ? 'training_id' : 'entry_id';
        ...
        unsubscribe = onSnapshot(q, (snapshot) => {
          ...
          commentsData.sort((a, b) => {
            ...
            const dateA =
              typeof createdA === 'object' &&
              createdA !== null &&
              typeof (createdA as { toDate?: () => Date }).toDate === 'function'
                ? (createdA as { toDate: () => Date }).toDate()
                : new Date(a.created_at as string | number | Date);
            const createdB = b.created_at as unknown;
            const dateB =
              typeof createdB === 'object' &&
              createdB !== null &&
              typeof (createdB as { toDate?: () => Date }).toDate === 'function'
