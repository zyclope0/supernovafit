...
export const UNIMPLEMENTABLE_CHALLENGES = {
  ...
  'Hydratation Parfaite': 'Nécessite tracking de l\'eau bue (fonctionnalité à développer)',
  'Hydratation Express': 'Nécessite tracking de l\'eau bue (fonctionnalité à développer)',
...
export function getUnimplementationReason(challengeTitle: string): string | null {
  return UNIMPLEMENTABLE_CHALLENGES[challengeTitle as keyof typeof UNIMPLEMENTABLE_CHALLENGES] || null
}
...
export function getImplementableTodos(): string[] {
  return CHALLENGE_DEFINITIONS
    .map(def => def.title)
    .filter(title => 
      isChallengeImplementable(title) && 
      !isChallengeImplemented(title)
    )
}
