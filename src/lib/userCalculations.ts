...
export function getSportCorrectionFactor(niveau_activite: string): number {
  switch (niveau_activite) {
    case 'sedentaire':
      return 0.9; // 90% du sport = nouveau (peu d'activité prévue)
    case 'leger':
      return 0.7; // 70% du sport = nouveau (activité légère prévue)
    case 'modere':
      return 0.5; // 50% du sport = nouveau (activité modérée prévue)
    case 'intense':
      return 0.3; // 30% du sport = nouveau (activité intense prévue)
    case 'tres_intense':
      return 0.1; // 10% du sport = nouveau (activité très intense prévue)
    default:
      return 0.7; // Défaut conservateur
  }
}
