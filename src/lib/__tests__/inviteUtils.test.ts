...
describe('inviteUtils', () => {
  ...
  describe('calculateExpirationDate', () => {
    ...
    it("devrait calculer une date d'expiration (72h)", () => {
      ...
      expect(Math.abs(expirationDate.getTime() - expectedTime)).toBeLessThan(
        1000,
      );
    });
    ...
    it('devrait utiliser 72 heures par défaut', () => {
      ...
      expect(
        Math.abs(defaultExpiration.getTime() - secondExpiration.getTime()),
      ).toBeLessThan(1000);
    });
  });
  ...
  describe('validateInviteCode', () => {
    it("devrait valider un code d'invitation correct", () => {
      const validCodes = ['ABC234', 'XYZ789', 'TEST23'];
