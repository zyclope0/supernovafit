...
function enhanceSearchQuery(query: string): string {
  ...
  return enhanced !== normalizedQuery
    ? `${normalizedQuery} ${enhanced}`
    : normalizedQuery;
}
...
export async function searchProducts(
  ...
  try {
    ...
    const smartQuery =
      FRENCH_SYNONYMS[normalizedQuery] ||
      enhanceSearchQuery(normalizedQuery) ||
      normalizedQuery;
    ...
    for (const queryTerm of queries) {
      ...
      url.searchParams.set(
        'fields',
        'code,product_name,brands,image_front_url,nutriments,categories_tags',
      );
    ...
    const fuse = new Fuse(candidates, {
      ...
      getFn: (obj: OpenFoodFactsProduct, path: string | string[]) => {
        ...
        return normalizeText(
          String((obj as unknown as Record<string, unknown>)[key] ?? ''),
        );
      },
    });
    ...
    const products = fuzzyMatched
      .map((p) => {
        ...
        let categoryBoost = 0;
        const tags =
          (p as unknown as { categories_tags?: string[] }).categories_tags ||
          [];
        ...
        const organicBoost = tags.some(
          (tag) =>
            tag.includes('organic') ||
            tag.includes('bio') ||
            tag.includes('biologique'),
        )
          ? 3
          : 0;
        ...
        const additivePenalty =
          name.includes('E') && /\bE\d{3}\b/.test(name) ? 5 : 0;
...
function transformProduct(
  ...
  const nutriments =
    (rawProduct as { nutriments?: Record<string, unknown> }).nutriments || {};

  return {
    code:
      (rawProduct as { code?: string; _id?: string }).code ||
      (rawProduct as { code?: string; _id?: string })._id ||
      '',
    product_name:
      (rawProduct as { product_name?: string; product_name_fr?: string })
        .product_name ||
      (rawProduct as { product_name?: string; product_name_fr?: string })
        .product_name_fr ||
      'Produit sans nom',
    ...
    image_url:
      (rawProduct as { image_front_url?: string; image_url?: string })
        .image_front_url ||
      (rawProduct as { image_front_url?: string; image_url?: string })
        .image_url ||
      '',
    nutriments: {
      energy_100g: (() => {
        ...
        const kcalValue = parseFloat(
          (nutriments as Record<string, unknown>)['energy-kcal_100g'] as string,
        );
        ...
        const kjValue = parseFloat(
          (nutriments as Record<string, unknown>).energy_100g as string,
        );
      ...
      proteins_100g:
        parseFloat(
          (nutriments as Record<string, unknown>).proteins_100g as string,
        ) || 0,
      carbohydrates_100g:
        parseFloat(
          (nutriments as Record<string, unknown>).carbohydrates_100g as string,
        ) || 0,
      fat_100g:
        parseFloat(
          (nutriments as Record<string, unknown>).fat_100g as string,
        ) || 0,
    },
  };
}
 ...
 */
export function hasCompleteNutritionalData(
