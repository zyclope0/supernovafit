...
export const repasSchema = z.object({
  ...
  repas: z.enum(['petit_dej', 'collation_matin', 'dejeuner', 'collation_apres_midi', 'diner', 'collation_soir'], {
...
export const entrainementSchema = z.object({
  ...
  type: z.enum(['cardio', 'musculation', 'course', 'cyclisme', 'natation', 'hiit', 'yoga'], {
  ...
  if (data.distance && data.duree && data.vitesse_moy) {
    ...
    const ecart = Math.abs(vitesseCalculee - data.vitesse_moy) / vitesseCalculee
  ...
  message: "La vitesse moyenne semble incohérente avec la distance et la durée",
  path: ["vitesse_moy"]
})
