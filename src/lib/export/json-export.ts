 ...
 */
function formatEntrainementsForJSON(
  ...
  return {
    ...
    summary: {
      ...
      totalCalories: entrainements.reduce(
        (sum, e) => sum + (e.calories || 0),
        0,
      ),
      averageDuration:
        entrainements.length > 0
          ? entrainements.reduce((sum, e) => sum + e.duree, 0) /
            entrainements.length
          : 0,
...
function formatMesuresForJSON(mesures: Mesure[]): Record<string, unknown> {
  ...
  return {
    ...
    statistiques: {
      ...
      periode:
        firstMesure && lastMesure
          ? {
              debut: format(new Date(firstMesure.date), 'yyyy-MM-dd', {
                locale: fr,
              }),
              fin: format(new Date(lastMesure.date), 'yyyy-MM-dd', {
                locale: fr,
              }),
            }
          : null,
...
function getMostFrequentMeal(repas: Repas[]): string {
  ...
  return (
    Object.entries(mealCounts).sort(([, a], [, b]) => b - a)[0]?.[0] || 'Aucun'
  );
}
...
function getMostFrequentTrainingType(entrainements: Entrainement[]): string {
  ...
  return (
    Object.entries(typeCounts).sort(([, a], [, b]) => b - a)[0]?.[0] || 'Aucun'
  );
}
