...
function addTitlePage(
  ...
  doc.text(`Total des enregistrements : ${totalRecords}`, 105, 120, {
    align: 'center',
  });
}
...
function addSummaryPage(
  ...
  const totalWorkoutCalories = entrainements.reduce(
  ...
  yPos += 8;
  doc.text(
    `• Calories consommées : ${totalCalories.toFixed(0)} kcal`,
    30,
    yPos,
  );
  yPos += 8;
  ...
  yPos += 8;
  doc.text(
    `• Calories brûlées : ${totalWorkoutCalories.toFixed(0)} kcal`,
    30,
    yPos,
  );
  yPos += 8;
  ...
  if (mesures.length >= 2) {
    ...
    yPos += 8;
    doc.text(
      `• Évolution : ${evolution > 0 ? '+' : ''}${evolution.toFixed(1)} kg`,
      30,
      yPos,
    );
  }
...
function addChartsPage(
  ...
  if (mesures.length >= 2) {
    ...
    doc.text(
      `📊 Évolution du poids : ${stats.weightEvolution > 0 ? '+' : ''}${stats.weightEvolution.toFixed(1)} kg`,
      20,
      yPos,
    );
    yPos += 15;
  ...
  if (repas.length > 0) {
    ...
    doc.text(
      `🍽️ Calories moyennes par jour : ${stats.avgCaloriesPerDay.toFixed(0)} kcal`,
      20,
      yPos,
    );
    yPos += 15;
  ...
  if (entrainements.length > 0) {
    ...
    doc.text(
      `💪 Durée moyenne par séance : ${stats.avgWorkoutDuration.toFixed(0)} min`,
      20,
      yPos,
    );
    yPos += 15;
...
function addChartToPDF(
  ...
  if (chartData.labels.length > 0) {
    ...
    doc.text(
      `Moy: ${(chartData.datasets[0].data.reduce((a, b) => a + b, 0) / chartData.datasets[0].data.length).toFixed(1)}`,
      x + width + 5,
      y + 30,
    );
  }
 ...
 */
function addEntrainementsSection(
  doc: jsPDF,
  entrainements: Entrainement[],
): void {
  doc.setFontSize(18);
...
function addRecommendationsPage(
  ...
  if (repas.length > 0) {
    const avgCalories =
      repas.reduce((sum, r) => sum + r.macros.kcal, 0) / repas.length;
    const avgProteins =
      repas.reduce((sum, r) => sum + r.macros.prot, 0) / repas.length;

    doc.text(`🍽️ Recommandations nutritionnelles :`, 20, yPos);
    yPos += 15;
    doc.text(
      `• Calories moyennes par jour : ${avgCalories.toFixed(0)} kcal`,
      30,
      yPos,
    );
    yPos += 8;
    doc.text(
      `• Protéines moyennes par jour : ${avgProteins.toFixed(1)}g`,
      30,
      yPos,
    );
    yPos += 8;
    ...
    if (avgCalories < 1500) {
      ...
      doc.text(
        `• ⚠️ Apport calorique élevé, surveillez vos objectifs`,
        30,
        yPos,
      );
    } else {
  ...
  if (entrainements.length > 0) {
    const avgDuration =
      entrainements.reduce((sum, e) => sum + e.duree, 0) / entrainements.length;
    const weeklyWorkouts = entrainements.length;

    doc.text(`💪 Recommandations d'entraînement :`, 20, yPos);
    yPos += 15;
    doc.text(
      `• Durée moyenne par séance : ${avgDuration.toFixed(0)} min`,
      30,
      yPos,
    );
    yPos += 8;
    ...
    if (weeklyWorkouts < 3) {
      doc.text(
        `• 💡 Considérez augmenter la fréquence d'entraînement`,
        30,
        yPos,
      );
    } else if (weeklyWorkouts > 6) {
      doc.text(
        `• 💡 Attention au surentraînement, prévoyez des jours de repos`,
        30,
        yPos,
      );
    } else {
