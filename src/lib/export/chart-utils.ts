...
export function generateCaloriesChartData(repas: Repas[]): ChartData {
  ...
  return {
    labels: sortedDays.map(([day]) =>
      format(new Date(day), 'dd/MM', { locale: fr }),
    ),
    datasets: [
      {
        label: 'Calories (kcal)',
...
export function generateMacrosChartData(repas: Repas[]): ChartData {
  ...
  const totalMacros = repas.reduce(
    (acc, r) => ({
    ...
    datasets: [
      {
        label: 'Macronutriments (g)',
        data: [
          totalMacros.proteines,
          totalMacros.glucides,
          totalMacros.lipides,
        ],
        backgroundColor: ['#3498db', '#e74c3c', '#f39c12'],
        borderWidth: 1,
 ...
 */
export function generateWorkoutFrequencyChartData(
...
export function calculateChartStatistics(
  ...
  const avgWorkoutDuration =
    entrainements.length > 0 ? totalWorkoutTime / entrainements.length : 0;
