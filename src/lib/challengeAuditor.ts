...
export class ChallengeAuditor {
   ...
   */
  static validateChallengeDates(challenge: Challenge): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
  ...
  static fixChallengeDates(challenge: Challenge): Partial<Challenge> {
    ...
    if (
      !challenge.startDate ||
      challenge.startDate === '' ||
      isNaN(new Date(challenge.startDate).getTime())
    ) {
    ...
    if (
      ...
      const startDate = new Date(
        updates.startDate || challenge.startDate || now,
      );
   ...
   */
  static validateChallengeDefinition(challenge: Challenge): {
    isValid: boolean;
    suggestion?: (typeof CHALLENGE_DEFINITIONS)[0];
  } {
    const definition = CHALLENGE_DEFINITIONS.find(
      (def) =>
        def.title === challenge.title ||
        (def.description === challenge.description &&
          def.type === challenge.type),
    );
  ...
  static auditChallenges(challenges: Challenge[]) {
    ...
    const summary = {
      ...
      unknownDefinitions: results.filter((r) => !r.definitionValidation.isValid)
        .length,
    };
