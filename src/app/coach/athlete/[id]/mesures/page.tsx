...
import { collection, query, where, onSnapshot, doc, getDoc, addDoc, serverTimestamp, limit } from 'firebase/firestore'
...
export default function CoachAthleteMesuresPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      q,
      (snapshot) => {
        const data = snapshot.docs.map((d) => ({ id: d.id, ...d.data() })) as Mesure[]
    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          <Link href={`/coach`} className="p-2 hover:bg-white/10 rounded-lg transition-colors">
            <ArrowLeft className="h-5 w-5 text-white" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold neon-text flex items-center gap-2">
              <Scale className="h-5 w-5 text-neon-green" /> Mesures — {athlete?.nom || athlete?.email}
            </h1>
            <p className="text-sm text-muted-foreground">Dernières mesures et commentaires</p>
          </div>
        </div>

        {/* Historique des mesures */}
        <CollapsibleCard title="Historique des mesures" defaultOpen counter={mesures.length}>
          {mesures.length === 0 ? (
            <div className="text-sm text-muted-foreground">Aucune mesure trouvée.</div>
          ) : (
            <div className="space-y-3">
              {mesures.map((m) => (
                <div key={m.id} className="glass-effect p-4 rounded-lg border border-white/10">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
                    {m.poids !== undefined && (
                      <div className="text-center">
                        <div className="text-lg font-semibold text-neon-green">{m.poids}</div>
                        <div className="text-xs text-muted-foreground">kg</div>
                      </div>
                    )}
                    {m.taille !== undefined && (
                      <div className="text-center">
                        <div className="text-lg font-semibold text-neon-cyan">{m.taille}</div>
                        <div className="text-xs text-muted-foreground">cm</div>
                      </div>
                    )}
                    {/* IMC calculé si données présentes */}
                    {m.poids !== undefined && m.taille !== undefined && m.taille > 0 && (
                      <div className="text-center">
                        <div className="text-lg font-semibold text-neon-purple">{(m.poids / Math.pow(m.taille / 100, 2)).toFixed(1)}</div>
                        <div className="text-xs text-muted-foreground">IMC</div>
                      </div>
                    )}
                    {m.masse_grasse !== undefined && (
                      <div className="text-center">
                        <div className="text-lg font-semibold text-neon-pink">{m.masse_grasse}</div>
                        <div className="text-xs text-muted-foreground">% MG</div>
                      </div>
                    )}
                    {m.masse_musculaire !== undefined && (
                      <div className="text-center">
                        <div className="text-lg font-semibold text-neon-green">{m.masse_musculaire}</div>
                        <div className="text-xs text-muted-foreground">% MM</div>
                      </div>
                    )}
                    {m.tour_taille !== undefined && (
                      <div className="text-center">
                        <div className="text-lg font-semibold text-neon-cyan">{m.tour_taille}</div>
                        <div className="text-xs text-muted-foreground">cm taille</div>
                      </div>
                    )}
                    {m.tour_hanches !== undefined && (
                      <div className="text-center">
                        <div className="text-lg font-semibold text-neon-purple">{m.tour_hanches}</div>
                        <div className="text-xs text-muted-foreground">cm hanches</div>
                      </div>
                    )}
                    {m.tour_poitrine !== undefined && (
                      <div className="text-center">
                        <div className="text-lg font-semibold text-neon-pink">{m.tour_poitrine}</div>
                        <div className="text-xs text-muted-foreground">cm poitrine</div>
                      </div>
                    )}
                  </div>
        ...
        {showCommentModal && selectedMesure && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="glass-effect rounded-2xl p-6 border border-white/10 max-w-lg w-full">
              <h3 className="text-xl font-semibold text-white mb-2">Commenter la mesure</h3>
              <p className="text-sm text-gray-400 mb-4">
                {new Date(selectedMesure.date).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}
              </p>
              <textarea
                value={newComment}
