...
export default function WeightIMCChart({ mesures, period = 'week' }: WeightIMCChartProps) {
  const chartData = useMemo(() => {
    ...
    if (period === 'today') {
      ...
      if (todayMesures.length > 0) {
        ...
        filteredMesures = mesures.filter(m => new Date(m.date) >= threeDaysAgo)
      } else {
      ...
      if (filteredMesures.length < 3) {
        ...
        filteredMesures = mesures.filter(m => new Date(m.date) >= twoWeeksAgo)
      }
      ...
      if (filteredMesures.length < 3) {
        ...
        filteredMesures = mesures.filter(m => new Date(m.date) >= startOfLastMonth)
      }
    }
  ...
  const motivationStats = useMemo(() => {
    const filteredWeights = chartData.data.map(d => d.poids).filter(Boolean) as number[]
    ...
    const periodStart = chartData.data.length > 0 ? new Date(chartData.data[0].date) : new Date()
    const periodDays = Math.max(1, Math.ceil((new Date().getTime() - periodStart.getTime()) / (1000 * 60 * 60 * 24)))
        ...
        <div className="mb-4 grid grid-cols-3 gap-2 text-xs">
            <div className="text-center">
              <div className={`text-lg font-bold ${
                motivationStats.totalChange < 0 ? 'text-green-400' : 
                motivationStats.totalChange > 0 ? 'text-red-400' : 'text-gray-400'
              }`}>
                {motivationStats.totalChange > 0 ? '+' : ''}{motivationStats.totalChange.toFixed(1)}kg
              </div>
          ...
          <div className="text-center">
            <div className={`text-lg font-bold ${
              motivationStats.percentChange < 0 ? 'text-green-400' : 
              motivationStats.percentChange > 0 ? 'text-red-400' : 'text-gray-400'
            }`}>
              {motivationStats.percentChange > 0 ? '+' : ''}{motivationStats.percentChange.toFixed(1)}%
            </div>
            <div className="text-white/60">Variation</div>
          </div>
          <div className="text-center">
            <div className={`text-lg font-bold ${
              motivationStats.recentTrend === 'loss' ? 'text-green-400' : 
              motivationStats.recentTrend === 'gain' ? 'text-red-400' : 'text-gray-400'
            }`}>
              {motivationStats.recentTrend === 'loss' ? '📉' : 
               motivationStats.recentTrend === 'gain' ? '📈' : '➡️'}
            </div>
            <div className="text-white/60">
              {period === 'today' ? 'Tendance' : period === 'week' ? 'Semaine' : 'Mois'}
            </div>
          </div>
        </div>
      )}
