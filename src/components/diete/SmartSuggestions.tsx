...
export default function SmartSuggestions({
  ...
  const hasSignificantGaps =
    gap.calories > 100 || gap.protein > 15 || gap.carbs > 30 || gap.fat > 10;

  if (!hasSignificantGaps || suggestions.length === 0) {
    return (
      <div
        className={`glass-effect rounded-xl p-4 border border-white/20 ${className}`}
      >
    ...
    >
                  ...
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span
                          className={`px-2 py-1 rounded-full text-xs flex items-center gap-1 ${getPriorityColor(suggestion.priority)}`}
                        >
                      ...
                      <p className="text-xs text-white/70 mb-2">
                        {suggestion.quantity} • {suggestion.estimatedCalories}{' '}
                        kcal
                      </p>
                    ...
                    <button
                      onClick={() =>
                        setExpandedSuggestion(
                          isExpanded ? null : suggestion.food.id,
                        )
                      }
                    ...
                    <button
                      onClick={() => {
                        ...
                        setAddedSuggestions(
                          (prev) =>
                            new Set(
                              Array.from(prev).concat(suggestion.food.id),
                            ),
                        );
                      }}
                      disabled={isAdded}
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className="flex justify-between">
                          ...
                          <span className="text-orange-400">
                            {suggestion.nutritionalValue.calories} kcal
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-white/70">Protéines:</span>
                          <span className="text-blue-400">
                            {suggestion.nutritionalValue.protein}g
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-white/70">Glucides:</span>
                          <span className="text-cyan-400">
                            {suggestion.nutritionalValue.carbs}g
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-white/70">Lipides:</span>
                          <span className="text-purple-400">
                            {suggestion.nutritionalValue.fat}g
                          </span>
                        </div>
                        {suggestion.nutritionalValue.fiber && (
                          <div className="flex justify-between">
                            ...
                            <span className="text-green-400">
                              {suggestion.nutritionalValue.fiber}g
                            </span>
                          </div>
                        )}
                        {suggestion.nutritionalValue.sodium && (
                          <div className="flex justify-between">
                            ...
                            <span className="text-yellow-400">
                              {suggestion.nutritionalValue.sodium}mg
                            </span>
                          </div>
                        )}
                      </div>
      ...
      <div className="mt-4 pt-3 border-t border-white/10">
        <p className="text-xs text-white/50 text-center">
          💡 Suggestions basées sur vos objectifs{' '}
