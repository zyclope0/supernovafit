      ...
      {plan ? (
        <div className="space-y-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-white">Plan Diététique</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { key: 'petit_dej', label: 'Petit Déjeuner', icon: '🌅', content: plan.petit_dej },
              { key: 'collation_matin', label: 'Collation Matin', icon: '☕', content: plan.collation_matin },
              { key: 'dejeuner', label: 'Déjeuner', icon: '🍽️', content: plan.dejeuner },
              { key: 'collation_apres_midi', label: 'Collation Après-midi', icon: '🍎', content: plan.collation_apres_midi },
              { key: 'diner', label: 'Dîner', icon: '🌙', content: plan.diner },
              { key: 'collation_soir', label: 'Collation Soir', icon: '🌃', content: plan.collation_soir }
            ].map(meal => (
              meal.content && (
                <div key={meal.key} className="p-4 bg-white/5 border border-white/10 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-lg">{meal.icon}</span>
                    <h4 className="text-sm font-medium text-neon-cyan">{meal.label}</h4>
                  </div>
          ...
          {plan.notes_generales && (
            <div className="p-4 bg-neon-purple/10 border border-neon-purple/20 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="text-sm font-medium text-neon-purple">Notes du Coach</h4>
              </div>
              <p className="text-sm text-gray-300 leading-relaxed">
                {plan.notes_generales}
              </p>
      ...
      {comments && comments.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-medium text-white">Commentaires Récents</h3>
            <span className="text-xs text-gray-400 px-2 py-1 bg-gray-800 rounded">
              {comments.length}
            </span>
          </div>
              ...
              <div key={comment.id} className="p-3 bg-white/5 border border-white/10 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-neon-cyan font-medium">
                    Coach • {new Date(comment.date || '').toLocaleDateString('fr-FR')}
                  </span>
