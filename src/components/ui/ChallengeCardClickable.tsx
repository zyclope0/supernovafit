...
import { Calendar, Clock, Target, Trophy, Play, Pause, Star } from 'lucide-react'
...
export default function ChallengeCardClickable({
  ...
  const getDifficultyColor = () => {
    switch (challenge.difficulty) {
      case 'easy': return 'bg-neon-green/10 border-neon-green/20'
      case 'medium': return 'bg-neon-yellow/10 border-neon-yellow/20'
      case 'hard': return 'bg-neon-red/10 border-neon-red/20'
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-white/10">
          <div className="text-xs text-gray-400">
            {challenge.status === 'active' && `${Math.round(progressPercentage)}% terminé`}
            {challenge.status === 'completed' && `Terminé le ${formatDate(challenge.completed_at || '')}`}
