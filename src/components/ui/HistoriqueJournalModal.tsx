...
export default function HistoriqueJournalModal({
  isOpen,
  onClose,
  allEntries,
  currentDate,
  onDateChange,
}: HistoriqueJournalModalProps) {
  ...
  const commentedEntryDates = useMemo(
    () =>
      new Set(
        (journalComments || [])
  ...
  const getStatsForDate = (date: string) => {
    ...
    const avgHumeur =
      count > 0
        ? Math.round(
            (dayEntries.reduce((s, e) => s + (e.humeur || 0), 0) / count) * 10,
          ) / 10
        : 0;
    const avgEnergie =
      count > 0
        ? Math.round(
            (dayEntries.reduce((s, e) => s + (e.energie || 0), 0) / count) * 10,
          ) / 10
        : 0;
  ...
  const headerContent = (
    <div className="flex bg-white/5 rounded-lg p-1">
      <button
        ...
        className={`px-3 py-1 rounded text-sm font-medium transition-colors ${viewMode === 'calendar' ? 'bg-neon-green/20 text-neon-green' : 'text-muted-foreground hover:text-white'}`}
      >
        <Calendar className="h-4 w-4" />
      </button>
      <button
        onClick={() => setViewMode('stats')}
        className={`px-3 py-1 rounded text-sm font-medium transition-colors ${viewMode === 'stats' ? 'bg-neon-green/20 text-neon-green' : 'text-muted-foreground hover:text-white'}`}
      >
        ...
        {viewMode === 'calendar' ? (
              ...
              {last30Days.map((date, idx) => {
                ...
                return (
                  <button
                    ...
                    aria-label={`${formatFullDate(date)}${hasData ? `, ${s.count} entrées, humeur ${s.avgHumeur}` : ', aucune donnée'}`}
                  >
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="glass-effect-high p-4 rounded-lg text-center glass-hover">
              ...
              <div className="glass-effect-high p-4 rounded-lg text-center glass-hover">
                ...
                <div className="text-sm text-accessible-secondary">Entrées</div>
              </div>
              <div className="glass-effect-high p-4 rounded-lg text-center glass-hover">
                <div className="text-2xl font-bold text-neon-purple">
                  {globalStats.totalDays > 0
                    ? Math.round(
                        (globalStats.avgHumeurSum / globalStats.totalDays) * 10,
                      ) / 10
                    : 0}
                </div>
                <div className="text-sm text-accessible-secondary">
              ...
              <div className="glass-effect-high p-4 rounded-lg text-center glass-hover">
                <div className="text-2xl font-bold text-neon-pink">
                  {globalStats.totalDays > 0
                    ? Math.round(
                        (globalStats.avgEnergieSum / globalStats.totalDays) *
                          10,
                      ) / 10
                    : 0}
                </div>
              ...
              {last30Days
                ...
                .map((date) => {
                  ...
                  return (
                    <div
                      key={date}
                      className={`glass-effect p-3 rounded-lg flex items-center justify-between cursor-pointer hover:bg-white/5 transition-colors ${isCurrent(date) ? 'border border-neon-green/30' : ''}`}
                      onClick={() => {
                        onDateChange(date);
                        onClose();
                      }}
