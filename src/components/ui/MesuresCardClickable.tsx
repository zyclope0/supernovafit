          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {stats.evolution_poids !== 0 && (
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`font-medium ${stats.evolution_poids > 0 ? 'text-neon-green' : 'text-neon-red'}`}
                >
                  {stats.evolution_poids > 0 ? '+' : ''}
                  {stats.evolution_poids.toFixed(1)} kg
                </span>
              </div>
            )}
            {stats.evolution_masse_grasse !== 0 && (
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`font-medium ${stats.evolution_masse_grasse > 0 ? 'text-neon-red' : 'text-neon-green'}`}
                >
                  {stats.evolution_masse_grasse > 0 ? '+' : ''}
                  {stats.evolution_masse_grasse.toFixed(1)}%
                </span>
              </div>
            )}
          </div>
