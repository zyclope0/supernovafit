...
export default function ModuleComments({ comments, loading, compact = false }: ModuleCommentsProps) {
  const [localComments, setLocalComments] = useState<CoachComment[]>(comments || [])
  useEffect(() => {
  ...
  const markAsRead = async (commentId: string) => {
    try {
      ...
      setLocalComments(prev => prev.map(c => c.id === commentId ? { ...c, read_by_athlete: true } : c))
    } catch {}
  }
  if (loading) {
    // Skeleton compact
    return (
      <div className="mt-4 space-y-2">
        {[0,1].map((i) => (
          <div key={i} className="bg-white/5 rounded-lg p-2 border border-white/5 animate-pulse">
    ...
    return (
      <div className="mt-4 space-y-3">
        {localComments.map((comment) => (
          <div key={comment.id} className={`bg-neon-purple/10 border rounded-lg p-3 ${comment.read_by_athlete ? 'border-white/10' : 'border-neon-purple/20'}`}>
            <div className="flex items-start space-x-2">
              <ChefHat className="w-4 h-4 text-neon-purple mt-0.5 flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-neon-purple font-medium text-xs">Commentaire Coach</span>
  ...
  return (
    <div className="mt-4">
      <div className="flex items-center space-x-2 mb-2">
        ...
        <span className="bg-neon-cyan/20 text-neon-cyan text-xs px-2 py-0.5 rounded-full">{comments.length}</span>
      </div>
      <div className="space-y-2 max-h-40 overflow-auto pr-1">
        {localComments.map((comment) => (
          <div key={comment.id} className={`bg-white/5 rounded-lg p-2 border ${comment.read_by_athlete ? 'border-white/10' : 'border-neon-pink/30'}`}>
            <div className="flex items-start space-x-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <span className="text-neon-purple font-medium text-xs">Coach</span>
