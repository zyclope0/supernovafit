...
export default function StatsDashboard({
  ...
  return (
    <div
      className={`glass-effect p-4 sm:p-5 lg:p-6 rounded-xl border border-white/10 bg-gradient-to-r from-neon-purple/5 to-neon-cyan/5 ${className}`}
    >
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3 md:gap-4 mb-4">
        {stats.map((stat, index) => {
          ...
          return (
            <div
              key={index}
              className={`text-center p-2 sm:p-3 rounded-lg ${colors.bg} border ${colors.border}`}
            >
