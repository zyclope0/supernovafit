...
function TrainingCardClickableComponent({
  training,
  onView,
  onEdit,
  onDelete,
}: TrainingCardClickableProps) {
  const trainingType =
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h3
                className={`font-medium text-${trainingType.color} group-hover:text-white transition-colors`}
              >
                {trainingType.label}
              </h3>
          ...
          {training.commentaire && (
            <div className="text-sm text-muted-foreground bg-white/5 p-3 rounded-lg">
              &quot;
              {training.commentaire.length > 100
                ? `${training.commentaire.slice(0, 100)}...`
                : training.commentaire}
              &quot;
            </div>
          )}
