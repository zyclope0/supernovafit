...
export default function CaloriesInOutChart({ repas, entrainements, days = 7, tdee }: CaloriesInOutChartProps) {
  ...
  function CustomTooltip({ active, payload, label }: any) {
    ...
    return (
      <div style={{ background: 'rgba(15,23,42,0.97)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: 10 }}>
        <div style={{ color: 'white', fontWeight: 600, marginBottom: 6 }}>{label}</div>
        <div style={{ color: '#a3a3a3', fontSize: 12 }}>Consommées: <span style={{ color: '#10b981' }}>{inVal} kcal</span></div>
        <div style={{ color: '#a3a3a3', fontSize: 12 }}>TDEE (base): <span style={{ color: '#60a5fa' }}>{tdeeVal} kcal</span></div>
        <div style={{ color: '#a3a3a3', fontSize: 12 }}>Sport: <span style={{ color: '#ec4899' }}>{sportVal} kcal</span></div>
        <div style={{ color: '#a3a3a3', fontSize: 12 }}>Dépensées totales: <span style={{ color: 'white' }}>{outTotal} kcal</span></div>
        <div style={{ color: netColor, fontSize: 12, marginTop: 4 }}>Bilan (In - Out): {net} kcal</div>
      </div>
    )
  }
  ...
  return (
    <div className="glass-effect p-6 rounded-xl border border-white/10">
      ...
      {typeof tdee === 'number' && tdee > 0 && (
        <p className="mt-2 text-xs text-muted-foreground">Le TDEE (bleu) est inclus dans les dépenses quotidiennes.</p>
      )}
    </div>
  )
}
