...
export default function AchievementCard({ 
  ...
  return (
    <div className={`glass-effect rounded-xl p-6 border transition-all duration-300 hover:scale-105 ${
      isUnlocked 
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className={`text-lg font-bold mb-1 ${isUnlocked ? 'text-white' : 'text-white/50'}`}>
              {achievement.name}
            </h3>
            <div className="flex items-center gap-2 text-sm">
              {getRarityIcon()}
              <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRarityColor(achievement.rarity)} bg-white/10`}>
                {achievement.rarity}
              </span>
            </div>
          </div>
      ...
      {/* Description */}
      <p className={`text-sm mb-4 leading-relaxed ${isUnlocked ? 'text-white/80' : 'text-white/40'}`}>
      ...
      {showDetails && (
        <div className="mb-4">
          ...
          <div className={`text-sm ${isUnlocked ? 'text-white/70' : 'text-white/40'}`}>
      ...
      {isUnlocked && achievement.rarity !== 'common' && (
        <div className={`absolute inset-0 rounded-xl bg-gradient-to-r ${getRarityGradient()} opacity-5 pointer-events-none`} />
      )}
    </div>
  )
}
