...
const CaloriesInOutChart = dynamic(
  () => import('@/components/ui/CaloriesInOutChart'),
  { ssr: false },
);
const MacrosChart = dynamic(() => import('@/components/ui/MacrosChart'), {
  ssr: false,
});
const WeightIMCChart = dynamic(
  () => import('@/components/charts/WeightIMCChart'),
  { ssr: false },
);
...
export default function DesktopDashboard({ className }: DesktopDashboardProps) {
  ...
  const periodDays =
    selectedPeriod === 'today' ? 1 : selectedPeriod === 'week' ? 7 : 30;
  ...
  const weightTrend =
    latestWeight && previousWeight
      ? latestWeight.poids! > previousWeight.poids!
        ? 'up'
        : latestWeight.poids! < previousWeight.poids!
          ? 'down'
          : 'stable'
  ...
  const quickStats: QuickStat[] = [
    {
      ...
      unit: 'kcal',
      trend:
        periodStats.calories > (estimatedTDEE || 2000) * 0.8 ? 'up' : 'down',
      trendValue:
        selectedPeriod === 'today'
    ...
    },
    {
      ...
      trendValue: previousWeight
        ? `${Math.abs(latestWeight!.poids! - previousWeight.poids!).toFixed(1)}kg`
        : undefined,
      color: 'purple',
      icon: Scale,
    },
    {
      ...
      unit:
        selectedPeriod === 'today' ? '' : selectedPeriod === 'week' ? '/7' : '',
      trend:
        periodTrainings.length >=
        (selectedPeriod === 'today' ? 1 : selectedPeriod === 'week' ? 3 : 8)
          ? 'up'
          : 'down',
      color: 'orange',
      icon: Dumbbell,
            ...
            <div className="glass-effect p-6 rounded-xl border border-white/10">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-white">
                  Balance Énergétique ({periodLabel})
                </h3>
            ...
            userProfile?.ownerCoachId && (
              <div className="glass-effect p-4 rounded-lg border border-white/10 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-white">Lié à un coach</span>
                </div>
                <button
                  onClick={() => {
                    toast(
                      'Pour changer de coach, contactez votre coach actuel',
                      {
                        icon: '💡',
                        duration: 4000,
                      },
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    {userProfile && energyBalance.rawSportCalories > 0 && (
                      <span
                        ...
                        title={`Correction sport: ${Math.round(avgDailySportCalories)}kcal/j × ${correctionFactor}`}
                      >
                  ...
                  <span className="text-sm text-neon-green">
                    {formatNumber(
                      selectedPeriod === 'today'
                        ? todayStats.calories
                        : periodStats.calories,
                    )}{' '}
                    / {formatNumber(estimatedTDEE || 2000)}
                  </span>
                </div>
                <div className="w-full bg-space-700 rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${Math.min(((selectedPeriod === 'today' ? todayStats.calories : periodStats.calories) / (estimatedTDEE || 2000)) * 100, 100)}%`,
                    }}
                  />
                </div>
              </div>
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm text-neon-cyan">
                    {formatNumber(
                      selectedPeriod === 'today'
                        ? todayStats.proteins
                        : periodStats.proteins,
                    )}{' '}
                    / {Math.round((latestWeight?.poids || 70) * 1.6)}g
                  </span>
                </div>
                <div className="w-full bg-space-700 rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${Math.min(((selectedPeriod === 'today' ? todayStats.proteins : periodStats.proteins) / Math.round((latestWeight?.poids || 70) * 1.6)) * 100, 100)}%`,
                    }}
                  />
                </div>
              </div>
